...
function renderPreview() {
  const { metadata, body } = parseFrontmatter(input.value);
  preview.srcdoc = renderPreviewDocument(metadata, renderMarkdown(body));
  wordCount.textContent = `${countWords(body)} parole`;
  renderValidation();
}

function renderPreviewDocument(metadata, content) {
